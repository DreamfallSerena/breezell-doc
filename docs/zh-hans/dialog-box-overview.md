...
指针突出显示输入托盘中的 **Quick Phrases（快捷短语）** 按钮。快捷短语可保存常用提示词或指令，之后无需重复输入即可在新消息中复用。

![输入托盘中的快捷短语按钮](/dialog-box/quick-phrases-button.png)

打开 **Quick Phrases** 后，使用 **Add** 创建短语。左侧 **All phrases** 显示全部已保存短语；将常用短语拖到右侧 **Favorites** 即可收藏，拖回左侧可取消收藏。

选择已保存的短语即可将其插入当前输入框。发送消息前仍可继续编辑短语内容。

![快捷短语管理界面](/dialog-box/quick-phrases-manager.png)

## MCP 服务器

指针突出显示输入托盘中的 **MCP** 按钮。打开后可以查看已配置的 MCP 服务器，并控制现有 MCP 服务器的启用状态。

![MCP 服务器控制面板](/dialog-box/mcp-server-controls.png)

每个服务器条目提供以下功能：

- **调用箭头：** 点击后会在当前输入框中插入 `/mcp:<服务器名称>`。发送包含该引用的请求，即可让 AI 调用所选 MCP 服务器。
- **启用开关：** 用于开启或关闭现有 MCP 服务器。关闭后，AI 将无法使用该服务器，直至重新开启。
- **健康状态：** 绿点表示 MCP 服务器已连接且运行正常；红点表示服务器不可用或状态异常，调用前应检查其配置或进程。

![输入框中插入的 MCP 引用](/dialog-box/mcp-invocation-reference.png)

## 技能

指针突出显示输入托盘中的 **Skills（技能）** 按钮。打开后可以管理 Breezell 可供 AI 使用的技能。

![官方内置技能面板](/dialog-box/skills-built-in.png)

技能面板分为两个选项卡：

- **Built-in（内置技能）：** Breezell 官方自带的技能，无需自行创建或配置即可使用。
- **My Skills（我的技能）：** 由你自行配置或创建的技能。

![我的技能面板](/dialog-box/skills-my-skills.png)

两个选项卡中的技能均可单独控制：

- **调用箭头：** 点击后会在当前输入框中插入 `@<技能名称>`。发送包含该提及的请求，即可让 AI 调用所选技能。
- **启用开关：** 用于开启或关闭技能。关闭后，AI 将无法使用该技能，直至重新开启。

![输入框中插入的技能提及](/dialog-box/skill-invocation-reference.png)
